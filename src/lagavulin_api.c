...
void LagavulinRegWrite( BYTE reg, BYTE value );
BYTE LagavulinRegRead( BYTE reg );
void LagavulinRegStopReceived( void );

void LagavulinPmicWrite( BYTE reg, BYTE value );
BYTE LagavulinPmicRead( BYTE reg );
void LagavulinPmicStopReceived( void );
...
void ApiInit( void )
{
    ...
    ApiRegisterHandlers( DEVID_ROCKET, LagavulinRegRead, LagavulinRegWrite, NULL, LagavulinRegStopReceived );
#endif // PLATFORM_RIMEVALBOARD

    // PMIC read/write passthrough
    ApiRegisterHandlers( DEVID_PMIC, LagavulinPmicRead, LagavulinPmicWrite, NULL, LagavulinPmicStopReceived );
...
BYTE LagavulinRegRead( BYTE reg )
{
    ...
    switch( reg ) {
        ...
        case ROCKET_REG_VERSION1:
            return( LagavulinMetrics.BuildVersionMaj );

        case ROCKET_REG_VERSION2:
            return( LagavulinMetrics.BuildVersionMin );
        ...
        case ROCKET_REG_HWV_HARDWAREID3:
            return( LagavulinMetrics.HwvHardwareId>>((reg-ROCKET_REG_HWV_HARDWAREID0)*8) );

        case ROCKET_REG_HWV_CLASS_PWRSEQ:
            return( LagavulinMetrics.HwvPwrSeq );

        case ROCKET_REG_BUILDINFO1:
            return( LagavulinMetrics.BuildInfoMaj );

        case ROCKET_REG_BUILDINFO2:
            return( LagavulinMetrics.BuildInfoMin );
        ...
        case ROCKET_REG_API_VERSION_MAJOR:
            return( LagavulinMetrics.BootImage->versionMaj );

        case ROCKET_REG_API_VERSION_MINOR:
            return( LagavulinMetrics.BootImage->versionMin );
...
BYTE LagavulinPmicRead( BYTE reg )
{
    ...
    if( (reg >= PMIC_MFIQ) && (reg <= PMIC_FIRQ) ) {
